...
{
	g_MyClrBgHi = LIST_BK_COLOR;
	g_MyClrFgHi = RGB(229,229,229);
	m_clrBK = INFOZONE_BK_COLOR;
	m_setColorflag = false;
}
...
void CSkinListCtrl::SetBKColor(COLORREF clr)
{
	m_clrBK = clr;
}

void CSkinListCtrl::SetItemColor(DWORD iItem, COLORREF color)
{
	m_setColorflag = true ;
...
void CSkinListCtrl::OnPaint() 
{
	...
	memDC.FillSolidRect(clip, m_clrBK);
	   
	SetTextBkColor(m_clrBK);
