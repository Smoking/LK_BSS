...
CQueryMedicine::CQueryMedicine(CWnd* pParent /*=NULL*/)
	: CRCDialog(CQueryMedicine::IDD, pParent)
{
...
void CQueryMedicine::OnPanDian() 
{
	...
	for(int i=0; i<m_listMedInfo.GetItemCount(); i++ )
	{
		if( m_listMedInfo.GetCheck(i) )
		{
			m_listMedInfo.GetItemText(i,7,buf,100);
			if(strcmp(buf,"未上架")==0)
			{
				m_listMedInfo.SetCheck(i,FALSE);
				Util::MessageBox(this->GetSafeHwnd(), "该药品没有上架不需要盘点");
				continue;
			}
			m_listMedInfo.GetItemText(i,1,buf,100);
			int medID=atoi(buf);
			...
			if(Util::MessageBox(this->GetSafeHwnd(), strText)==IDOK)
			{
				...
				if(sum_count>0)
				{
					bizser->clearMedicine(medID,1,sum_count);//药品盘点
					med->clearAllLocation(medID);
					Utils::insertSystemlog("2106","盘点药品地址",buf);
					m_listMedInfo.GetItemText(i,5,buf,100);		
					m_listMedInfo.SetItemText(i, 5, _T(Utils::itos(atoi(buf)-sum_count).c_str()));
